Gem::Specification.new do |s|
  s.name         = 'country-select'
  s.version      = '1.1.2'
  s.date         = '2012-09-03'
  s.summary      = 'Country select box'
  s.description  = 'Provides a form helper to insert a country select box using the ISO 3166 country list.'
  s.authors      = [ 'Michael Koziarski', 'James Dean Shepherd', 'Dale Campbell' ]
  s.email        = [ 'jamesds2007@gmail.com', 'oshuma@gmail.com' ]
  s.files        = [ 'lib/country-select.rb',
                     'README.md',
                     'MIT-LICENSE' ]
  s.homepage     = 'http://github.com/Oshuma/country-select'
end
